...
export default async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    switch (method) {
        case "GET":
            try {
                const query = 'SELECT * FROM customerRate WHERE id=$1';
                const values = [id];
        ...
        case "PUT":
            try {
                ...
                const text =
                    "UPDATE customerRate SET idCustomer = $1, idRate = $2, factor = $3 WHERE id = $4 RETURNING *";
                const values = [idCustomer, idRate, factor, id];
        ...
        case "DELETE":
            try {
                const text = "DELETE FROM customerRate WHERE id = $1 RETURNING *";
                const values = [id];
                const result = await cnn.query(text, values);
